...
export function getSvgIconFailedToSanitizeRawError(raw: SafeHtml): Error {
  return Error(
    `The raw provided to IconRegistry was not trusted as safe HTML by ANGULAR's DOMSanitizer. Attempted raw was : ${
      raw as string
    }`
  );
}

export function getSvgIconFailedToSanitizeUrlError(
  url: SafeResourceUrl
): Error {
  return Error(
    `The URL provided to IconRegistry was not trusted as a resource URL via Angular's DOMSanitizer. Attempted URL was : ${
      url as string
    }`
  );
}
...
export function getSvgIconNoHttpClientProvidedError(): Error {
  return Error(
    `Could not fid HttpClient provider for use with core-angular icons. Please include the HttpClientModule from @angular/common/http in your app imports`
  );
}
...
export class IconRegistry implements OnDestroy {
  ...
  public addSvgIconInNamespace(
    ...
    return this._addSvgIconConfig(
      namespace,
      iconName,
      new SvgIconConfig(url, undefined, options)
    );
  }
  ...
  public addRawSvgIconInNamepsace(
    ...
    return this._addSvgIconConfig(
      namespace,
      iconName,
      new SvgIconConfig('', trustedRaw, options)
    );
  }
  ...
  private _getSvgIconFromConfig(config: SvgIconConfig): Observable<SVGElement> {
    if (config.svgText) {
      return of(
        cloneSvg(this._svgElementFromConfig(config as LoadedSvgIconConfig))
      );
    } else {
